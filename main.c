...
void load_assets(struct engine* e) {
    ...
    free(b);
    if (d) { e->img.pixels = (uint32_t*)d; e->img.width = w; e->img.height = h; }
}
...
void android_main(struct android_app* app) {
    ...
    while (1) {
        ...
        if (e.init && app->window) {
            e.x += e.joy.dx * 10.0f; e.y += e.joy.dy * 10.0f;
            ...
            if (ANativeWindow_lock(app->window, &b, NULL) == 0) {
                ...
                if (e.img.pixels) graphics_draw_image(&rb, &e.img, (int)e.x, (int)e.y);
                ui_draw(&rb, &e.joy);
