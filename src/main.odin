...
handle_app_command :: proc "c" (app: ^Android_App, command: c.int32_t) {
    ...
    case APP_CMD_INIT_WINDOW:
        if app.window != nil {
            ...
            ANativeWindow_setBuffersGeometry(app.window, c.int32_t(width), c.int32_t(height), WINDOW_FORMAT_RGBA_8888)
            if !game.initialized {
    ...
    case APP_CMD_WINDOW_RESIZED:
        if app.window != nil {
            game_resize(game, int(ANativeWindow_getWidth(app.window)), int(ANativeWindow_getHeight(app.window)))
        }
...
handle_input_event :: proc "c" (app: ^Android_App, event: ^AInputEvent) -> c.int32_t {
    ...
    case AMOTION_EVENT_ACTION_MOVE:
        ...
        if game.attack.pressed && !ui_button_contains(&game.attack, x, y) {
            game.attack.pressed = false
        }
...
android_main :: proc "c" (app: ^Android_App) {
    // An exported Odin procedure is entered from C, so initialize Odin's
    // runtime context explicitly before calling regular Odin procedures.
    ...
    for {
        ...
        ident := ALooper_pollOnce(0, nil, nil, cast(^rawptr)&source)
        for ident >= 0 {
            if source != nil {
        ...
        if ANativeWindow_lock(app.window, &native_buffer, nil) == 0 {
